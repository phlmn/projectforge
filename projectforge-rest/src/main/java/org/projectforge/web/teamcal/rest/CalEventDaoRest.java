...
import org.projectforge.rest.calendar.converter.CalRecurrenceEvent;
...
/**
 * REST interface for {@link CalEventDao}
 *
 * @author Kai Reinhard (k.reinhard@micromata.de)
 */
@Controller
@Path(RestPaths.TEAMEVENTS)
public class CalEventDaoRest
{
  private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(CalEventDaoRest.class);

  @Autowired
  private CalEventServiceCopy calEventService;

  @Autowired
  private TeamCalCache teamCalCache;

  private ICSConverter converter = new ICSConverter();
  ...
  {
    log.info("Call rest interface CalEventDaoRest.getReminderListPastAndFuture - BEGIN");
    Calendar start = null;
    ...
    if (cals.size() > 0) {
      ...
      final List<CalEventDO> list = calEventService.getTeamEventICSDOList(filter);
    ...
    log.info("Call rest interface CalEventDaoRest.getReminderList - END");
    return Response.ok(json).build();
  }

  /**
   * Rest call for {@link CalEventDaoCopy#getEventList(TeamEventFilter, boolean)}
   *
   * @param calendarIds  The id's of the calendarIds to search for events (comma separated). If not given, all calendarIds
   *                     owned by the context user are assumed.
   * @param daysInFuture Get events from today until daysInFuture (default is 30). Maximum allowed value is 90.
      ...
      @QueryParam("modifiedSince") final Integer daysInFuture)
  {
    ...
    if (cals.size() > 0) {
      ...
      final List<ICalendarEvent> list = calEventService.getEventList(filter, true);
      if (list != null && list.size() > 0) {
  ...
  private Response handleICal(final CalendarEventObject calendarEvent)
  {
    try {
      ...
      // Get handler
      final ICSHandler handler = this.calEventService.getEventHandler(teamCalDO);
  ...
  private CalendarEventObject getEventObject(final ICalendarEvent src)
  {
    ...
    final ICSGenerator generator = ICSGenerator.Companion.exportAllFields();
    ...
    if (src instanceof CalEventDO) {
      CalEvent calEvent = converter.convertToCalEvent((CalEventDO) src);
      copyFields(event, calEvent);
    ...
    if (src instanceof CalRecurrenceEvent) {
      final CalEvent master = ((CalRecurrenceEvent) src).getMaster();
      if (master != null) {
    ...
    calEvent.setEndDate(new Timestamp(src.getEndDate().getTime()));
    calEvent.setLocation(src.getLocation());
    calEvent.setNote(src.getNote());
    calEvent.setStartDate(new Timestamp(src.getStartDate().getTime()));
    calEvent.setSubject(src.getSubject());
    calEvent.setUid(src.getUid());
    calEvent.setAllDay(src.getAllDay());
  ...
  private void copyFields(final CalendarEventObject event, final CalEvent src)
  {
    event.setCalendarId(src.getCalendar().getId());
    ...
    DOConverter.copyFields(event, converter.convertToDO(src));
